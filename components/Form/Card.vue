...
<template>
	<div class="p-4">
		<UForm :state="formState" :schema="cardSchema" class="-mt-4" :class="type === 'update'
			...
			: ''
			" @submit="onSubmit">
			<div v-if="type === 'update'" class="md:w-[60%] w-full space-y-3 pt-4 h-[55vh] overflow-y-auto md:border-b-0 border-b border-white md:pb-0 pb-8">
				<div>
					<h3>Title</h3>
				...
				<div>
					<h3>Due Date</h3>
					<div class="flex items-center w-max p-1 rounded-sm mt-1 ml-4" :class="formState.isDone
						? 'bg-green-500'
						: status === 'todo'
							? 'bg-blue-500'
							: status === 'inprogress'
						...
						<div v-for="comment in data" class="flex items-start gap-2 mb-2">
							...
							<div>
								...
								<div class="bg-white rounded-md p-2 shadow dark:bg-gray-700 ">
									<p class="leading-none text-black break-all dark:text-white">
										{{ comment.message }}
									</p>
								</div>
								<div class="flex items-center gap-2 mt-1">
									<p class="text-[11px] underline cursor-pointer">edit</p>
									<p class="text-[11px] underline cursor-pointer">delete</p>
								</div>
							</div>
						</div>
					</div>
